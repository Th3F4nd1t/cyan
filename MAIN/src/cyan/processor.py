...
from .memory import *
from .config import *
from .dynamic_resources.instructions import *
import copy
import os, sys
...
class Processor:
    ...
    def generate_state(self, config): #resets and takes all state from config.yaml
        ...
        if self.static.pipelined:
            self.pipeline = self.Pipeline(config["pipeline"],config)
    ...
    def load_program(self, instructions, config):
        ...
        # no more duplicate classes that was a pain
        # if any inheritance issues pop up again its probably not here
        for index,line in enumerate(instructions):
            ...
            self.state.prom.append(eval(f"{opcode.upper()}(index)"))
            instr_class = self.state.prom[-1]
        ...
        def init_forwarder(self):
            ...
            for component in self.config["components"]:
                if component["forwarded"]:
                    self.forwarder[component["class"].upper()] = [None for _ in range(len(self.stages))]
